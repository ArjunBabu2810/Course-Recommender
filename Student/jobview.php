...
<a href="jobrating.php"><button class="update" style="margin:2em;">Rate Your Job</button></a>

    <div class="course-container">
        <?php
        require '../connection.php';
        $sql = "SELECT * FROM Job ORDER BY Rating DESC";
            ...
            while ($row = mysqli_fetch_array($data)) {
                if ($row['Status'] == 1) {
                    ...
                    echo '<div class="course-box">
                            <h3>'.$row['Name'].'</h3>
                            <p><strong>ID:</strong> '.$row['Jobid'].'</p>
                            <p><strong>Prequality:</strong>';
                            $course1=$row['Courseid'];
                    
                    $sql3 = "SELECT Courseid, Name FROM Course WHERE Courseid=$course1";
                    ...
                    $sql2 = 'SELECT Student_id, Job_id FROM Intrested WHERE Student_id='.$_SESSION['id'].' AND Job_id='.$row['Jobid'].'';
                        //echo $sql2;
                        $data2 = mysqli_query($dbcon, $sql2);

                        if ($data2) { // Check if the query was successful
                            if(mysqli_num_rows($data2)>0){
                                echo '<a href="interested.php?Jobid='.$row['Jobid'].'"><button class="update">Marked</button></a>';
                            }else{
                                echo '<a href="interested.php?Jobid='.$row['Jobid'].'"><button class="update">Interested</button></a>';
